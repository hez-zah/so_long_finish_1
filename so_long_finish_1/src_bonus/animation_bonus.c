...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/03/09 20:59:16 by sellyani          #+#    #+#             */
/*   Updated: 2025/03/09 21:00:18 by sellyani         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	put_war_img(t_map **slong)
{
	...
	while (++i < (*slong)->war_collect)
	{
		mlx_put_image_to_window((*slong)->mlx, (*slong)->mlx_win,
			(*slong)->img, (*slong)->coll[i].y_collect * SIZE,
			(*slong)->coll[i].x_collect * SIZE);
	}
}
...
void	render_img(t_map **slong)
{
	...
	(*slong)->img = mlx_xpm_file_to_image((*slong)->mlx, SPACE, &img_w, &img_h);
	put_war_img(slong);
	(*slong)->img = mlx_xpm_file_to_image((*slong)->mlx,
			(*slong)->e_var.imgs[(*slong)->e_var.x], &img_w, &img_h);
	put_war_img(slong);
}
